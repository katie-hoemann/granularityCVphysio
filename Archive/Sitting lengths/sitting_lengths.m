...
%     figure;
%     edges_sec = 0:100:5600;
%     histogram_sec = histogram(subjectData,edges_sec);
%     set(gca,'YScale','log');
%     ylim([0.95 2500]);
%     title(['Seated rest duration (sec): subject ' char(subjectIDs(i_subject))]);
%     figureName = [char(subjectIDs(i_subject)) '_rest_duration_sec'];
%     saveas(histogram_sec,figureName,'tiff');
    
%     figure;
%     subjectData_min = round((subjectData/60),1);
%     edges_min = 0:5:85;
%     histogram_min = histogram(subjectData_min,edges_min,'BinWidth');
%     set(gca,'YScale','log');
%     ylim([0.95 2500]);
%     title(['Seated rest duration (min): subject ' char(subjectIDs(i_subject))]);
%     figureName = [char(subjectIDs(i_subject)) '_rest_duration_min'];
%     saveas(histogram_min,figureName,'tiff');

    figure;
    subjectData_min = round((subjectData/60),1);
    edges_min = [0 .5 1 5 10 30 90];
    [bins,edges] = discretize(subjectData_min,edges_min);
    [counts(i_subject,:),~] = histcounts(subjectData_min,edges_min);
    catCounts = categorical(bins,[1 2 3 4 5 6],{'<30s','30s-1m','1-5m','5-10m','10-30m','>30m'});
