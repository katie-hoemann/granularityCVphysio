...
save_summary_name = participant_directory + 'sitting_features_summary_results_setNandA_' + s_id + '.xlsx'
model_file = participant_directory + 'model_setNandA_' + s_id + '.pk'
...
if clustering_method == 'DPGMM':
    for r in range(RANDOM_RESTARTS):
        model = BayesianGaussianMixture(n_components=N_COMPONENTS,max_iter=300,verbose=False,random_state=r,covariance_type=COVARIANCE_TYPE,weight_concentration_prior=ALPHA).fit(feature_matrix)
        avg_ll[r] = np.sum(model.score(feature_matrix))
        print (avg_ll[r])
    model = BayesianGaussianMixture(n_components=N_COMPONENTS,max_iter=300,verbose=True,random_state=np.argmax(avg_ll),covariance_type=COVARIANCE_TYPE,weight_concentration_prior=ALPHA).fit(feature_matrix)
