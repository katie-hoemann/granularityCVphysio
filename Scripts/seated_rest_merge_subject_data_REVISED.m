% revised to extract mean posterior probability of cluster membership
...
for i_file = 1:length(files) % for every file
    ...
    probData = zeros(size(eventData,1),15); % number of columns based on max number of clusters + 1 (for cluster ID)
    probDataSpan = size(rawData.data.Sheet1(:,30:end),2); % get number of columns required
    probData(:,1:probDataSpan) = rawData.data.Sheet1(:,30:end); % insert posterior probability data into empty matrix big enough to accommodate all participants
    ...
    subjectIDprob = subjectID*ones(size(probData,1),1); % create subject ID column for posterior probability data
    probData = horzcat(subjectIDprob,probData); % add subject ID column to posterior probability data
    if i_file == 1
        eventDataCompile = eventData; % if it's the first participant, then don't merge with anything
        clusterDataCompile = clusterData;
        probDataCompile = probData;
    else
        eventDataCompile = vertcat(eventDataCompile,eventData);
        clusterDataCompile = vertcat(clusterDataCompile,clusterData);
        probDataCompile = vertcat(probDataCompile,probData);
    end
    clear eventData clusterData probData;
end

%% check data, add headers, and export data
...
probDataCompile_clean = probDataCompile(~isnan(probDataCompile(:,2)),:); % drop unclustered events
...
for i_subject = 1:length(subjectIDlist)
    ...
    subjectData = probDataCompile_clean(index1,:);
    numClusters(i_subject) = max(subjectData(:,2)+1);
    for i_event = 1:size(subjectData,1)
        clusterID(i_event) = subjectData(i_event,2);
        clusterProbs(i_event) = subjectData(i_event,clusterID(i_event)+3);
    end
    prob_M(i_subject) = mean(clusterProbs);
    clear clusterProbs;
end
...
eventDataCompile_agg = [subjectIDlist numberEvents' numberDays' RSA_grandMean' RSA_grandSD' IBI_grandMean' IBI_grandSD' PEP_grandMean' PEP_grandSD'...
    LVET_grandMean' LVET_grandSD' CO_grandMean' CO_grandSD' RMSSD_grandMean' RMSSD_grandSD' RR_grandMean' RR_grandSD' prob_M'];
aggColumnNames = {'PPID' 'numEvents' 'numDays' 'RSA_M' 'RSA_SD' 'IBI_M' 'IBI_SD' 'PEP_M' 'PEP_SD' 'LVET_M' 'LVET_SD' 'CO_M' 'CO_SD' 'RMSSD_M' 'RMSSD_SD' 'RR_M' 'RR_SD' 'Prob_M'};
